...
|| I5 | PR #166 | **Photo Only crop fallback + per-mode edit state.** Photo Only now extracts from card if `photoImageData` is null. Switching Full Document ↔ Photo Only retains each mode's slider values (both applied on A4). Back buttons added on password + editor steps. | **merged** |
|| I6 | PR #167 | **Photo Only dynamic photo detection.** Replaces hardcoded crop with dynamic pixel-density photo detection for Photo Only mode. | **merged** |
|| I7 | PR #168 | **PAN-specific card crop + photo detection.** Single `CARD_CROP` replaced with per-doc-type crops: `AADHAAR_CROP` (unchanged) + `PAN_CROP` (`{x0:0.06, y0:0.755, x1:0.955, y1:0.955}`). Photo detection x-scan limited to 22% for PAN (avoids QR code false positive), 45% for Aadhaar unchanged. Same editor flow works for both doc types. | **merged** |
