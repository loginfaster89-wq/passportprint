...
1. **F1** — Razorpay `undefined` URL (blocker, payments-adjacent).
2. **F2** — Google One-Tap auto-prompt on mobile (covers Login / content).
3. **F4** — Hero headline wrap at 820 / 390 (quick `text-wrap:balance` or explicit break).
4. **F3** — Features grid height / slider activation at 820 (either widen slider threshold or let primary span in 2-col).
