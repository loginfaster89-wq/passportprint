...
Everything from PRs #70–#193 is shipped. See `issues.md` for the full
...
- PR #192: docs update (shipped summary for #190–#191 + new prompt).
- PR #193: **ID Card Print Phase 2 multi-card A4 layout** —
  layout toggle in Step 3 (1 Pair Fold & Laminate vs 5 Pairs Multi-Card,
  default multi). 2 cols × 5 rows = 5 front+back pairs per A4. Dashed
  cut guides + L-shaped corner marks. Cards NOT mirrored (flat cut
  workflow). Responsive, print CSS updated.
...
Phase 2 remainder (rounded borders, Voter ID, Dragon sheet) + Phase 3
items — see lists above.
...
Last PR: PR #194 (docs: update shipped summary for PR #193 + new prompt)

Shipped summary (don't redo):
PRs #70–#194: all previous work shipped
...
- Phase 2 remainder OPEN: rounded borders, Voter ID, Dragon sheet
