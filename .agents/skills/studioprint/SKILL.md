...
Last shipped: PR #263 (feat: §AC.16 PAN crop fix to actual card row,
Aadhaar-parity layout). Companion: PR #262 (§AC.15 flush-top +
black-top-line) — both apply uniformly via type-agnostic
`buildSingleSheet`.
...
§AC.11–§AC.16 are all SHIPPED. Step 2 preview uses a unified
...
seam ticks, and **both Aadhaar AND PAN** cards crop correctly to
the actual card row (the formal Letter is dropped) with
Aadhaar-parity treatment.
